...
class MainWindow(QMainWindow):
    def __init__(self,*args,**kwargs):
        ...
        if getSettingBool('enableDownloads'):
            ...
            self.download_requested = lambda self,item:item.ignore()
        ...
        if getSettingBool('startBrowserMaximized'):
            tabs.setWindowState(Qt.WindowMaximized)
        x,y = getSettingSize(2,'tabSize')
        tabs.setStyleSheet('QTabBar::tab { height: %spx; width: %spx}'%(y,x))
        x,y,dx,dy = getSettingSize(4,'windowGeometry')
        tabs.setGeometry(x,y,dx,dy)
        
        tabs.tabCloseRequested.connect(self.remove_tab)
        ...
        if getSettingBool('enableIconInTopLeft'):
