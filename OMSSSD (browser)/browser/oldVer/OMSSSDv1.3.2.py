...
class MainWindow(QMainWindow):
    def __init__(self,*args,**kwargs):
        ...
        navtb.removeAction(QAction(QIcon('ico/ic.jpg'),'kur',navtb))
        self.addToolBar(navtb)

        if getSettingBool('enableBackBtn'):
            back_btn = QAction(QIcon('ico/zad.jpg'),'Back',self)
            back_btn.setStatusTip('Back to the previous page')
            back_btn.triggered.connect(self.browser.back)
            navtb.addAction(back_btn)

        if getSettingBool('enableForwardBtn'):
            next_btn = QAction(QIcon('ico/pred.jpg'),'forward',self)
        ...
        add_tab_btn = QAction(QIcon('ico/ic.jpg'),'Add a new tab',self)
...
def create_new_tab():
    tabs.addTab(MainWindow(),'New Tab')
